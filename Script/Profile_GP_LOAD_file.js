...
// Включить/выключить повторные попытки при ошибке по ТН.
const ENABLE_RETRY = true;
// Количество ПОВТОРНЫХ попыток после первой (1 = максимум 2 попытки всего).
const MAX_RETRIES = 1;
// Задержка перед повторным запросом, если по ТН пришла ошибка (HTTP, success:false, исключение).
const RETRY_DELAY_ON_ERROR_MS = 1500;
...
async function runCollectProfiles(tabNums) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      const retries = ENABLE_RETRY ? Math.max(0, Math.floor(MAX_RETRIES)) : 0;
      const maxAttempts = 1 + retries;
      let r = null;

      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          r = await fetchProfileByTN(tn);
        } catch (reqErr) {
        ...
        console.warn(
          ...
          RETRY_DELAY_ON_ERROR_MS,
          "мс"
        );
        if (RETRY_DELAY_ON_ERROR_MS > 0) {
          await delay(RETRY_DELAY_ON_ERROR_MS);
        }
      }
