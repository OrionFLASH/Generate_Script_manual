...
- Над вкладками работает **единая строка действий** (кнопки с иконками), и набор кнопок меняется по активной вкладке:
  ...
  - **Редактирование:** `⬇ Загрузить параметры`, `✏ Обновить параметр`, `🗂 Обновить из файла`, `🧩 Сформировать шаблон Payload`, чекбокс **«Автоэкранирование кавычек»**, фильтр `parameterType` для шаблона.
...
| **3.14** | Автоэкранирование кавычек (тоггл «Автоэкранирование кавычек»); `parameterValue` в файле может быть объектом `{…}`; repair loose JSON при разборе файла; **preflight** всего файла (create/update) с `JSON.parse(parameterValue)` до первого POST; фильтр `parameterType` только для `🧩 Сформировать шаблон Payload` (пустой фильтр = все типы). |
