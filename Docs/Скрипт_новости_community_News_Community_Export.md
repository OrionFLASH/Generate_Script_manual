...
2. После исчерпания попыток — **пропуск** этой страницы и переход к следующей (`pageNum+1`), пока есть страницы по `page.total` / лимиту.
3. Аварийная остановка — только если **два подряд** запроса исчерпали попытки **и** следующей страницы уже нет (конец комбинации или `total` неизвестен). Уже загруженное сохраняется; в `exportMeta.fatalError` — комбинация, `pageNum`, текст ошибки.
4. Итоговый цвет статистики: **зелёный** только без ошибок; при пропусках/ошибках — **красный** (`готово с ошибками` или `ошибка — сохранено`).
...
| **2.8** | Ретраи: не рвать пагинацию посреди `total`; зелёный статус только без ошибок |
