...
Живой блок «Статистика работы»: фаза, текущая комбинация, `newsStatus`, `businessBlock`/теги, страница `pageNum/total`, прогресс комбинаций, **собрано** (уже выгруженные строки новостей), **`newsCount`** (общее число новостей комбинации из `body.newsCount` первого ответа; рядом с `page.total`), ошибки/пропуски.
...
| **2.8** | В статистике: `newsCount` из первого ответа комбинации; подпись счётчика выгрузки — «собрано» |
