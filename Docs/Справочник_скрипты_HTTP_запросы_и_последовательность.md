...
**Назначение:** обновление старого скрипта новостей в единой модалке: выгрузка news + создание + смена статусов + базовое редактирование.
...
1. Вкладка «Выгрузка»: выбор status/block/tag, POST-пагинация и сохранение JSON.
2. Вкладки операций: загрузка JSON (файл/поле) и разбор в кандидаты.
3. Показ компактного списка с чекбоксами выбора (по умолчанию выбраны все).
4. Предвалидация критичных полей.
5. Подтверждение пользователем.
6. Последовательная отправка POST и сохранение `*_result_*.json`.
