...
|----------|-------------------|
| **Search → empInfoFull** | **Фаза 1:** для **каждого** ввода по очереди — POST search (все страницы). **После всех search:** сохранение JSON ответов и CSV «что искали» / `employeeId` (по строке на каждый hit). **Фаза 2:** пауза «после всех Search» (панель) → для **каждого уникального** UUID в порядке первого появления по всем поискам — GET empInfoFull (пауза «между запросами» между GET). |
...
При старте выполняется автоопределение пары стенд/контур по `window.location.origin`; если совпадение не найдено — fallback на `PROM/ALPHA`.
...
**Последовательность:** перед формой/файлом — **`ensureCachesForCreateOperation()`**: при необходимости один раз `POST` списка `ACTUAL` (кэш **`parameterCode`**) и при отсутствии типов из API — детализация **`parameterTypes`** (кэш допустимых типов, обновление селекта на вкладке «Создание»). Валидация → при необходимости проверка, что **`parameterCode`** ещё не в ACTUAL (иначе сообщение и **без** `param-create`) → подтверждение в широком модальном окне (все поля payload) → **`POST …/param-create`**. Из файла: тот же разбор, что в документе по скрипту (в т.ч. блоки `{...}`); при таком разборе поддерживается автоматическое удаление висячих запятых перед `}`/`]`, а при ошибке в журнал выводится расширенная диагностика (блок, позиция, строка/колонка, фрагмент). Пауза **`PARAM_BATCH_REQUEST_GAP_MS`** между запросами, дубли по коду отфильтровываются. Успех: **`success === true`**.

Дополнительно по единой кнопке **`⬇ Загрузить параметры`** (рядом со вкладками): **всегда** `POST` списка `ACTUAL`; шаг детализации мета-параметра `parameterTypes` (`POST { objectIds: [metaId] }`) применяется для вкладки «Создание». Применение результатов: для «Создания» `parameterType` и `businessBlock` дополняют дефолтные списки уникальными значениями из API, для «Редактирования» типы заменяются значениями из шага 1 (`ACTUAL`) без шага 2, а `businessBlock` заполняется по данным шага 1. Для `businessBlock` применяется fallback-список (8 значений: `KMKKSB`, `MNS`, `SERVICEMEN`, `KMFACTORING`, `KMSB1`, `IMUB`, `RNUB`, `RSB1`).

### 4.3. Вкладка «Редактирование» (`param-update`)

| Шаг | Метод | Путь | Payload |
|-----|--------|------|---------|
| (подготовка) | **POST** | `/bo/rmkib.gamification/proxy/v1/parameters` | `{ "status": "ACTUAL" }` — при отсутствии кэша или по кнопке ⬇ на вкладке 3: кэши **`parameterCode`**, **`objectId`**, списки для `parameterType`/`businessBlock` и карты связей |
...
**Последовательность:** при необходимости автоматически тот же поток, что и кнопка ⬇ вкладки 3 (**`ensureEditTabListsForUpdate`**) → проверки: **`objectId`** ∈ сохранённому множеству из шага ACTUAL, **`parameterCode`** ∈ кэшу кодов (иначе — указание создавать на вкладке 2) + сверка связки `objectId <-> parameterCode` по картам кэша → детализация B → подтверждение существующей комбинации `objectId + parameterCode` по ответу API → проверка `version` (если введена в форме/файле, обязана совпасть с API) → сравнение редактируемых полей `parameterType/businessBlock/parameterName/parameterValue/status` (если отличий нет, `param-update` не отправляется) → подтверждение в широком модальном окне (только `parameterCode` и изменённые поля) → шаг C.  
Для `parameterCode` в UI доступен поиск по части текста (`input + datalist`). В редактировании работают зависимые фильтры: `parameterType -> parameterCode`, `businessBlock -> parameterCode`, `businessBlock -> parameterType`, а также комбинация `businessBlock + parameterType -> parameterCode`; при невалидном фильтре показывается полный список кодов. При выборе кода или вводе `objectId` сначала автоподставляются `objectId/parameterCode`, `parameterType`, `status`, `version` из кэша 7.2, затем выполняется детализация **`POST { "objectIds": [ "<id>" ] }`** для предзаполнения `parameterName` и `parameterValue` (и уточнения сопутствующих полей).  
...
6. Ретраи: до **2** попыток на запрос (по умолчанию); при успехе на любой попытке — продолжение и сброс счётчика подряд; после исчерпания — пропуск страницы/комбинации; авария только если **2 подряд** запроса исчерпали все попытки. Зелёный статус только без ошибок. CSV-разделитель `;`.

На панели — статистика (status, block, **теги**, **собрано**, **newsCount**, **повторов**, **ошибок**) и **журнал/трейс** сразу под параметрами. JSON/CSV неактивны без status/block. Подробности: [Docs/Скрипт_новости_community_News_Community_Export.md](Скрипт_новости_community_News_Community_Export.md) (**v3.3**).
...
2. Вкладка «Выгрузка»: выбор status/block/tag, POST-пагинация только по указанному диапазону `pageNum` с общими ретраями/паузами и сохранение **двух JSON**: полный исходный + нормализованный `*_create_edit_template.json` (`createItems` / `updateItems`). При аварии/Стопе сохраняются только успешные страницы (`_partial`).
...
**Поведение:** между шагами выдерживается минимальная пауза (`STEP_DELAY_MS`); после клика ожидается загрузка страницы (`document.readyState`, смена URL, таймаут `NAVIGATION_TIMEOUT_MS` для SPA без смены `href`). В консоль пишутся метки OK / НЕ OK и итоговые счётчики.

**Запросы к API:** не выполняются (навигация только через UI страницы).
...
**Запросы к API:** скрипт **не** вызывает `fetch` к backend; трафик идёт как при обычной навигации браузера после клика.
...
| **1.29** | § **6** `News_Community_Export.js` v3.0: сводный релиз (newsCount, Закрыть busy, фикс ретраев, валидация, журнал). |
...
| **1.34** | § **6A** `News_Community_Export_v2.js`: добавлена вкладка выгрузки (старый блок), дефолт `createdBy=00673892`, явные кнопки выбора JSON-файла. |
...
| **1.38** | § **6A** `News_Community_Export_v2.js`: при ошибке/Стопе выгрузки сохранять JSON/CSV только из успешных страниц (`_partial`). |
| **1.39** | § **6A** `News_Community_Export_v2.js`: при выгрузке сохраняются два JSON — полный исходный и шаблон create/edit (`createItems` + `updateItems`). |
