...
6. Ретраи: до 3 попыток на запрос; при успехе на любой попытке — продолжение и сброс счётчика подряд; после исчерпания — пропуск страницы/комбинации; авария только если **2 подряд** запроса исчерпали все попытки. Зелёный статус только без ошибок. CSV-разделитель `;`.

На панели — статистика (status, block, **теги**, **собрано**, **newsCount**, **повторов**, **ошибок**) и **журнал/трейс** сразу под параметрами. JSON/CSV неактивны без status/block. Подробности: [Docs/Скрипт_новости_community_News_Community_Export.md](Скрипт_новости_community_News_Community_Export.md) (**v3.2**).
