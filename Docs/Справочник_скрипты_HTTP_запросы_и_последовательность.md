...
Дополнительно по единой кнопке **`⬇ Загрузить параметры`** (рядом со вкладками): **всегда** `POST` списка `ACTUAL` и **один** `POST` детализации мета-параметра `parameterTypes` (стенд-зависимый `objectId`: `PROM -> 745250143248942718`, `PSI/IFT-SB/IFT-GF -> 737634462490874360`). Результаты этой загрузки применяются ко всем вкладкам (типы, коды, бизнес-блоки, кэши сверок). Если шаг детализации не дал данные, поток не останавливается: остаются значения шага ACTUAL; для `businessBlock` применяется fallback-список (8 значений: `KMKKSB`, `MNS`, `SERVICEMEN`, `KMFACTORING`, `KMSB1`, `IMUB`, `RNUB`, `RSB1`).
