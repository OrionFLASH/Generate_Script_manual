...
**После создания:** новости создаются в `draft`; ID берётся из ответа (`body.objectId`, с запасными путями). Сразу под кнопками запуска показывается список созданных черновиков с чекбоксами — можно **опубликовать выбранные** той же функцией смены статуса, что и вкладка «Статусы» (`newsUpdate`, `method: "patch"`, `status: "published"`), или пропустить.
