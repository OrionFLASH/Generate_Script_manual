...
Над вкладками — общий блок настроек для **всех** операций (выгрузка / создание / статусы / редактирование / удаление):
...
| Кнопка ⏸ Пауза / ▶ Продолжить | Глобально приостанавливает отправку новых запросов во всех вкладках и режимах до ручного снятия паузы |

Кнопка **⏹ Стоп** на панели (и на вкладке выгрузки) прерывает текущий пакет после текущего запроса.  
Кнопка **⏸ Пауза** (в верхней строке операций) не отменяет прогон, а замораживает отправку следующих запросов до нажатия **▶ Продолжить**. Рядом со статусом операций всегда виден индикатор **«Пауза: ВКЛ» / «Пауза: ВЫКЛ»**. Одновременно может идти только одна операция.

Под строкой Пауза/Стоп — **блок статуса работы** (как в Pulse):

| Поле | Назначение |
|------|------------|
| Запрос | Текущий номер / всего |
| OK / Ошибки / Повторы / Пропуск | Счётчики пакета |
| Фаза | Создание / Выгрузка / Статусы / … |
| Сейчас | Текущий API-запрос и что обрабатывается |

Нижний блок переименован в **«Ошибки / критичное»**: обычный ход операции в журнал не пишется (он в статусе и Trace), в UI остаются ошибки, стоп, аварии и предупреждения.
