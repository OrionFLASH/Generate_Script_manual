...
Вкладки — компактная строка **сверху**; журнал и кнопки Trace вынесены ближе к общему блоку параметров, чтобы убрать пустой разрыв в интерфейсе. Подсказки в окнах — мелкий шрифт (10px).
...
### Нормализация загружаемого JSON в шаблон createItems

Для режима «Файл JSON» добавлена нормализация любого входа (и экспортный JSON, и шаблонный JSON):

- после разбора файл валидируется по критичным полям create (`type`, `createdBy`, `description`; для `achievement` также `rewardList` и `tournamentList`);
- данные приводятся к шаблонному виду `createItems[]` (лишние поля удаляются);
- нормализованный шаблон автоматически сохраняется в файл `news_create_template_normalized_*.json`;
- по кнопке **«Создать шаблон из загруженного»** можно в любой момент принудительно сохранить текущий нормализованный набор.
...
Дополнительно после завершения create сохраняется файл `news_create_template_with_ids_*.json`, где у элементов шаблона проставляется `createdObjectId` (если запись создана успешно). Это позволяет сразу использовать результат как рабочий шаблон для следующих операций.
...
Для источника из файла добавлена та же логика нормализации:

- вход приводится к шаблону `updateItems[]`;
- выполняется базовая валидация (`newsId` обязателен);
- после разбора автоматически сохраняется `news_edit_template_normalized_*.json`;
- по кнопке **«Создать шаблон из загруженного»** можно скачать нормализованный шаблон вручную.
...
| 1.x | 2026-08-05 | Create/Edit: нормализация входного JSON в шаблон (`createItems`/`updateItems`), авто/ручное сохранение шаблона; после create — шаблон с `createdObjectId`; журнал и Trace подтянуты к блоку параметров |
