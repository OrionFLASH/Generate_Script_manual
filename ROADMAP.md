...
- `[v]` **README.md** — описание структуры, ссылка на [Docs/Справочник_скрипты_HTTP_запросы_и_последовательность.md](Docs/Справочник_скрипты_HTTP_запросы_и_последовательность.md), перечень скриптов, история версий репозитория.
  - `[v]` Периодическая сверка списка скриптов в README с фактическим содержимым `Script/*.js` (сейчас **13** файлов + `Script/lib/DevToolsTrace.js`).
...
- `[v]` **Статистика на панели:** фаза, status, block, **теги**, страница `N/total`, прогресс, **собрано**, **newsCount**, **повторов**, **ошибок**.
...
- `[v]` Пагинация: `pageNum`, остановка по `isLast` и `total`; обход комбинаций `newsStatus × businessBlock` (+ опциональный `newsTagList`).
...
- `[v]` Валидация: ≥1 `newsStatus` и ≥1 `businessBlock` обязательны; теги опциональны.
- `[v]` **Стоп:** флаг `stopRequested`; выход после текущего POST; сохранение частичных JSON/CSV; `exportMeta.stoppedByUser`.
- `[v]` **Ретраи:** полный цикл попыток на pageNum; skip и продолжение пагинации при известном `total`; авария только когда следующей страницы нет; `done_ok` только при `errors === 0`.
- `[v]` **UI v2.5:** компактная сетка статус/блок/теги + ряд таймингов (payload/страницы/повтор/попытки).
- `[v]` **Лимит страниц:** `MAX_PAGES_PER_COMBO` / поле «Макс. страниц» (0 = все по total).
- `[v]` **Валидация запуска:** без выбранного статуса или блока выгрузка не стартует; теги могут быть пустыми.
- `[v]` **Журнал/трейс:** сразу после параметров (без зазора), растягивается по высоте; в статистике отдельная ячейка «теги».
- `[v]` **newsCount:** из `body.newsCount` — ячейка статистики; «собрано» — уже выгруженные строки.
- `[v]` **Закрыть:** неактивна во время выгрузки (`setExportButtonsBusy`).
- `[v]` **Счётчики retries/errors:** повтор = повторный запрос (attempt>1); ошибка = исчерпаны все попытки; в фазе — «повтор #N»; JSON/CSV disabled без status/block; подсветка пустых блоков.
- `[v]` **Логика стопа (v3.2):** авария только после двух подряд запросов с исчерпанными попытками (независимо от наличия следующей страницы); успех (в т.ч. с N-й попытки) сбрасывает счётчик подряд.
- `[v]` **RETRY_MAX по умолчанию = 2** (v3.3).

### 6.5 Доработка v3.4 (создание/обновление/статус + импорт из JSON)

- `[v]` Новый файл скрипта `Script/News_Community_Export_v2.js`:
  - `[v]` Вкладка «Создание»: загрузка JSON-шаблона/экспортного JSON + предпросмотр списка новостей с чекбоксами выбора (по умолчанию все выбраны).
- `[v]` Вкладка «Создание»: форма полей для **одной** новости; несколько — только через файл.
  - `[v]` Отправка `newsCreate` только для валидных и отмеченных записей; при ошибках критичных данных выполнение отменяется.
  - `[v]` Вкладка «Статусы»: смена `draft ↔ published` из интерфейса и из файла с подтверждением.
  - `[v]` Вкладка «Редактирование»: базовый каркас получения/выбора/подготовки редактирования + шаблон файла.
  - `[v]` Документация: профильный `Docs/*`, строка в README и обновление справочника HTTP.

### 6.6 Режим «болванка» при создании

- `[v]` Вкладка «Создание»: режим создания без `leadersList` и `authorsList` (игнор значений из файла и интерфейса).
  - `[v]` Чекбокс режима на панели создания.
...
### 6.8 Общие паузы/ретраи/Стоп на всех вкладках

- `[v]` Общий блок настроек пауз и повторов на уровне панели.
- `[v]` Ретраи + пауза между запросами + Стоп для создания / статусов / редактирования.
- `[v]` Выгрузка использует те же общие настройки.

### 6.9 Диапазон страниц выгрузки (Стр. с / Стр. по)

- `[v]` Поля **Стр. с** и **Стр. по** вместо «Макс. страниц».
- `[v]` В payload уходит только `pageNum` из диапазона (пример: 10…20).
- `[v]` `Стр. по = 0` — до последней страницы API (`isLast` / `total`).
- `[v]` Тот же диапазон учитывается при загрузке списка для редактирования.
- `[v]` При `body.page.isLast=true` следующий `pageNum` не запрашивается.

### 6.10 DevToolsTrace (сквозной + маскирование)

- `[v]` Встроен `DevToolsTrace` в `News_Community_Export_v2.js` (один переключатель над общим журналом).
- `[v]` Один буфер trace на все вкладки (выгрузка / создание / статусы / редактирование).
- `[v]` Маскирование ПДн в HTTP/LOG: ТН, ФИО, createdBy, sberChatMention, alpha/sigma ссылки; вложенный `newsFeature`.
- `[v]` Документация: профильный Docs v2 + `Docs/DevToolsTrace.md`; lib обновлён опцией `sanitizeForTrace`.

### 6.11 UI: вкладки сверху, рабочая область на всю ширину

- `[v]` Кнопки вкладок — компактная горизонтальная строка сверху (вместо боковой колонки).
- `[v]` Контент вкладки, блок статистики и журнал занимают всю доступную ширину панели.

### 6.12 Создание: achievement + разбор файла + публикация черновиков

- `[v]` Для `type=achievement` обязательны `rewardList` и `tournamentList` (форма и валидация перед create).
- `[v]` Коды из выгрузки: `rewards[].rewardCode`, `contests[].tournaments[].tournamentCode`.
- `[v]` Разбор файла: все `pages[]` / `comboResults[].pages` / `merged` (не только первая страница); в выгрузке в combo пишется `pages`.
- `[v]` После create — список `objectId` с выбором и кнопкой опубликовать (`patch` → `published`).
...
### 10.2a Каталог и просмотр bundle (фаза 7)

- `[v]` **Скачать с сервера:** `fetchAllParameters` — полный каталог tenant; поиск по части имени; выбор из списка + ручной ввод.
- `[v]` **Файл export:** колонка id; клик строки → панели «Свойства bundle» и «Values»; «Подтянуть id для отмеченных».
- `[v]` Документация: [Docs/Скрипт_SUP_Config_Update.md](Docs/Скрипт_SUP_Config_Update.md) v1.3 (сценарий F, § I.7–I.9).
...
- `[v]` Паузы с панели: между search-страницами, после всех Search, между mainInfo; номинал **1500 мс** + джиттер.
...
- `[v]` Все константы/дефолты — в блоке `PULSE_CFG` вверху скрипта.
- `[v]` Full / CSV зависят от Search+mainInfo (без обеих фаз недоступны).
- `[v]` Пауза по умолчанию **1500 мс** + джиттер (+5–10%; каждые 10/50 запросов +15–25%).
- `[v]` Дефолтные query: `673892`, `Лакомкин Олег`.
- `[v]` Trace: маскирование чувствительных полей ответов и значений в LOG (`X***xxx`).
...
| `News_Community_Export_v2.js` | Скрипт_новости_community_News_Community_Export_v2.md | § 6.5 |
