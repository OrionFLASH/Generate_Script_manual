...
- `[v]` **Единые элементы управления длинными выгрузками** (2026-08-10): во всех скриптах AddressBook / File_DB / News / Profile / Pulse / Leaders — живая панель статуса процесса, **⏸ Пауза / ▶ Продолжить**, **⏹ Стоп** с сохранением уже собранных данных (`_partial` где применимо).
...
- `[v]` Вкладка «Создание»: режим создания без `leadersList` и `authorsList` (игнор значений из файла при отправке).
  - `[v]` Чекбокс режима на панели создания.
  - `[v]` При включении — в запрос уходят пустые `leadersList` / `authorsList`.
  - `[v]` Счётчики авторов/лидеров в таблице выбора не меняются при включении/снятии болванки.
  - `[v]` Документация режима в `Docs/Скрипт_новости_community_News_Community_Export_v2.md`.
  - `[v]` Кнопки создания: цветные группы (Файл / Выбор / Запуск) и disabled по состоянию списка/выбора/операции.
...
- `[v]` В списке выбора: если `summary` пустой — первые 50 символов `newsText`/`description`.
- `[v]` В таблице создания/редактирования: NewsType, число тегов, коды reward/tournament (по типу, несколько через перевод строки).
...
### 6.10 Частичное сохранение выгрузки при ошибке/стопе

- `[v]` При аварии / Стоп / исключении сохранять JSON (и CSV) только из успешно загруженных страниц.
- `[v]` Ошибочные ответы в файл не класть; в `exportMeta` — флаги `stoppedByUser` / `abortedByErrors` / `partial`; суффикс `_partial`.

### 6.15 Нормализация загружаемых JSON для create/edit + фиксация созданных ID

- `[v]` Любой входной JSON (выгрузка или шаблон) валидировать по обязательным полям для create/edit.
- `[v]` Приводить вход к шаблонному формату (`createItems` / `updateItems`) и удалять лишние поля.
- `[v]` При **загрузке** данных для создания — нормализовать и сохранять шаблон; при create из файла новый шаблон не генерировать «на лету».
- `[v]` После `newsCreate` — файл шаблона только по успешным: в `createItems` пишутся новые `objectId`/`newsId` из ответа.
- `[v]` Сразу после создания — блок выбора черновиков для публикации (новые ID + общий `patchNewsStatus` / вкладка «Статусы»).
- `[v]` UI: журнал — последний блок под контентом вкладок, без пустого зазора после параметров (Trace у журнала).

### 6.16 Выгрузка: полный JSON + шаблон create/edit

- `[v]` При скачивании выгрузки сохранять два файла:
  - полный исходный JSON (`exportMeta` / `pages` / `merged`);
  - нормализованный шаблон с `createItems` и `updateItems` для вкладок Создание / Редактирование.
- `[v]` Шаблон формируется из успешно выгруженных новостей (включая partial).
- `[v]` Документация: профильный Docs + README + ROADMAP.

### 6.17 Восстановление UI/Trace после ошибочного merge

- `[v]` Причина: merge PR #21 через `--ours` затёр Trace/вкладки сверху/full-width/Удаление из `main` (b7fdc11).
...
- `[v]` Журнал — последний блок панели (после вкладок/контента), без пустого промежутка после блока параметров.

### 11. Python-утилита замены employeeNumber по CSV-справочнику

- `[v]` Один файл `.py` + `config_<name>.json`: чтение JSON из каталога `IN`, запись в `OUT`.
- `[v]` Сбор и дедуп `employeeNumber`; биективная замена на пул табельных из CSV (фильтры `BUSINESS_BLOCK=KMKKSB`, `ROLE_CODE=KM_KKSB`, `UCH_CODE=1`).
- `[v]` Единое соответствие: один исходный табельный → один заменяющий во всех файлах/вхождениях; без повторного использования заменяющих.
- `[v]` Замена связанных полей в блоке сотрудника: `lastName/firstName/gosbCode/tbCode/terDivisionName`.
- `[v]` `terDivisionName` по маппингу `tbCode -> name` из конфига; выходные файлы с префиксом `REPL_EmpID_`.
- `[v]` Запуск без обязательного `--config`: автопоиск `config_emp_replace.json` / `config_*.json` рядом со скриптом.
- `[v]` В конфиге явный список входных файлов `input_files` (один или несколько имён в `IN`).
- `[v]` После замен — Excel-отчёт `excel_report_file` (по умолчанию `REPL_EmpID_mapping.xlsx`): фиксированный заголовок + автофильтр (`openpyxl`).
- `[v]` Excel «было/стало» по всем заменяемым полям (`employeeNumber`, `lastName`, `firstName`, `gosbCode`, `tbCode`, `terDivisionName`); порядок: все «было» → все «стало» → ID новостей → файлы (несколько значений — через перевод строки).
...
- `[v]` Экспорт JSON / `.log` (тоглы состава) / `_ui.log`; связь `sessionId` / `eventId` / `corrId` / `afterUiEventId`.
- `[v]` Режим **Play**: накопление `_test_` без автосохранения; модалки dialog/popover/react-select.
- `[v]` Вкладка **FULL** (v1.12): HTML/CSS снимок + полный HTTP; без UI; бинарные тела — SKIP_BINARY.
