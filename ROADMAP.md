...
- `[v]` Добавить кнопку «Создать шаблон из загруженного» и использовать нормализованные данные для дальнейшей работы.
- `[v]` После `newsCreate` дописывать полученные `objectId` к соответствующим элементам шаблона и сохранять отдельный файл.
